...
//        }
     ...
     public static int SendRequest(Request request)
    {
        try {
  String url = "http://bhartiyamattress.com/buildLavatory.php?latitude="+request.getLat()+"&longitude="+request.getLong()+"&city="+request.getCity().getCityName().toUpperCase()+"&country="+request.getCountry().getCountryName().toUpperCase()+"&state="+request.getState().getStateName()+"&reqno="+request.getRequestNo();
  URL obj = new URL(url);
  HttpURLConnection con = (HttpURLConnection) obj.openConnection();
  int responseCode = con.getResponseCode();
  System.out.println("\nSending 'GET' request to URL : " + url);
